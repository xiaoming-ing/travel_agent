...
- `TravelIntent`、结构化意图识别、失败降级和长期偏好输入。
- 对话图中的意图节点、地点冲突、预算口径和关键歧义追问。
- 必去地点逐个查询、主题与标准偏好合并、避开地点确定性过滤。
- Phase 2 硬约束/软偏好分层，以及原始自由文本的不可信边界。
- 结果校验、跨天重复安全修复、最多一次定向模型修正和未满足项提示。
- 完成/修改/问答/未知反馈分类与路由。
- 常用景点、酒店、节奏、交通、饮食和预算修改工具。
- 修改反馈同步更新 `TravelIntent`，修改完成后重新执行约束校验。
- 10 条意图评测样例及准确率、平均耗时评测脚本。

验证状态：

- 离线测试全部通过；真实模型测试保留为独立集成验证，不进入日常单测。
- 真实模型约 90% 准确率目标需在允许访问模型服务的环境中运行评测脚本确认。
